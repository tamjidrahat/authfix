...
import sketchGen.analyzer.visitor.LibraryVisitor;
import sketchGen.analyzer.visitor.MethodVisitor;
import sketchGen.transformer.generator.ObsoleteExpressionGenerator;

public class OverloadMutator extends Transformer {
	...
	public OverloadMutator(TransformInstance subject) {
		super(subject);
		transformerName = "ov";
	}
	...
	public void visit(NameExpr expr, Void arg) {
		...
		String type = subject.getOriginClass().resolveVarType(subject.getBugLocation().getMethod(), expr);
	...
	private void transformOneConstructor(List<String> origTypes, NodeList<Expression> originParam, Class[] param) {
		...
		for (Class c : param) {
			...
			if (!typeCandidates.containsKey(name))
				typeCandidates.put(name, ObsoleteExpressionGenerator.fetchEXP(subject, name));
		}
	...
	private void fetchLibOverload(MethodCallExpr call) {
		...
		if (exp != null && (exp instanceof NameExpr)) {
			...
			String clazz = subject.getOriginClass().resolveVarType(subject.getBugLocation().getMethod(), n);
	...
	private List<String> resolveTypes(NodeList<Expression> origParam) {
		...
		for (int i = 0; i < origParam.size(); i++) {
			...
			if (!typeCandidates.containsKey(type))
				typeCandidates.put(type, ObsoleteExpressionGenerator.fetchEXP(subject, type));
		}
		return types;
	}
